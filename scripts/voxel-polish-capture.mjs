/** Focused production voxel-polish captures; fixed audit seed and held RAF clock.
...
const out = resolve(process.argv[2] ?? 'captures/voxel-polish');
...
try {
  ...
  await setup(page, 'ascent', 5);
  await page.evaluate(() => {
    const { game: g, renderer: r } = window.__refraction;
    g.board.clearAll();
    for (let x = 0; x < 8; x++)
      for (let y = 0; y < 5; y++) g.board.fill({ x, y, z: 7 - ((x + y * 3) % 8) });
    g.active.y = 10;
    r.render(g, 0);
  });
  ...
  results.push(
    await page.evaluate(() => {
      ...
      return { yaw: r.yaw, flatness: r.flatness };
    })
  );
