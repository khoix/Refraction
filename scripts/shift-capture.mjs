/** Focused production shift captures; fixed audit seed and held RAF clock.
...
const out = resolve(process.argv[2] ?? 'captures/shift');
...
try {
  ...
  await advance(page);
  await page.keyboard.press(process.argv[3] === 'left' ? 'ArrowLeft' : 'ArrowRight');
  for (const [name, frames] of [
    ['early', 10],
    ['mid', 12],
    ['late', 12],
    ['settled-destination', 12],
  ]) {
    ...
    results.push(
      await page.evaluate((name) => {
        ...
        return {
          ...
          yaw: r.yaw,
          flatness: r.flatness,
          elevation: r.renderedElevation,
          projection: r.camera.projectionMatrix.elements,
        };
      }, name)
    );
  }
  writeFileSync(resolve(out, 'measurements.json'), JSON.stringify(results, null, 2));
  await context.close();
} finally {
  await browser.close();
  server.kill();
